...
function ReelOverlay({ column, turbo, braking, finalSymbols }: {
  column: number;
  turbo: boolean;
  braking: boolean;
  ...
  const symbols = [...REEL_STRIP, ...REEL_STRIP, ...REEL_STRIP, ...REEL_STRIP, ...REEL_STRIP, ...finalSymbols];

  useEffect(() => { brakingRef.current = braking; }, [braking]);
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      if (itemHeight <= 0) { frameId = requestAnimationFrame(draw); return; }
      if (!initialized) { offset = itemHeight * REEL_STRIP.length; initialized = true; }
      const finalOffset = itemHeight * (symbols.length - 3);
      if (brakingRef.current) {
        if (reducedMotion()) { track.style.transform = `translateY(${-finalOffset}px)`; return; }
        if (brakeStartedAt === null) { brakeStartedAt = time; brakeStartOffset = offset; }
  ...
  return (
    <div ref={overlayRef} className={cn("gt-hw-reel-overlay", braking && "is-braking")} style={{ left: `${column * 33.333333}%` }} aria-hidden>
      <div ref={trackRef} className="gt-hw-reel-track" style={{ height: `${(symbols.length / 3) * 100}%`, animation: "none", transform: "translateY(0)" }}>
        {symbols.map((symbol, index) => (
          <div className="gt-hw-reel-item" style={{ height: `${100 / symbols.length}%` }} key={`${column}-${index}`}>
...
function FeatureCellMotion({ selectedSymbol, index }: { selectedSymbol: GoldenTigerSymbolId; index: number }) {
  return (
    <div className="gt-hw-cell-motion" aria-hidden>
      <div>{[selectedSymbol, "wild", selectedSymbol].map((symbol, item) => <span key={`${index}-${item}-${symbol}`}><GoldenTigerSymbol id={symbol} /></span>)}</div>
    </div>
  );
}
...
export function GoldenTigerPremium() {
  ...
  useEffect(() => { hydrateFromStorage(); return () => { autoStopRef.current = true; }; }, []);
  useEffect(() => { setGameAmbience("tiger", soundEnabled); return () => setGameAmbience("tiger", false); }, [soundEnabled]);
  useEffect(() => {
    const energy = phase === "full-grid" ? 1.48 : phase === "win" ? 1.34 : featureActive ? 1.3 : phase === "base-spin" ? 0.96 : 0.76;
  ...
  const animateFeature = useCallback(async (plan: FortuneFeatureResult, purchased = false) => {
    ...
    for (const step of plan.steps) {
      ...
      playGoldenTigerAudio({ type: "feature-respin", attempt: step.respin, lockedCount: lockedBeforeSpin }, soundEnabled);
      ...
      if (step.addedIndices.length > 0) {
        ...
        playGoldenTigerAudio({ type: "feature-lock", lockedCount: lockedAfterSpin, addedWild, fullGrid: step.isFullGrid }, soundEnabled);
  ...
  const settle = useCallback(async (payout: number, visualStake: number, accountingStake: number, note: string, fullGrid: boolean) => {
    ...
    if (fullGrid) {
      ...
      setPhase("win");
      playGoldenTigerAudio({ type: "win", tier: resolvedTier === "none" ? "small" : resolvedTier }, soundEnabled);
      await wait(turbo ? 280 : resolvedTier === "super" ? 2000 : resolvedTier === "mega" ? 1700 : resolvedTier === "big" ? 1350 : 650);
    } else if (payout > 0) {
      setPhase("return");
  ...
  const resetRoundPresentation = () => {
    setWin(0); setWinTier("none"); setWinning(new Set()); setFreshCells(new Set()); setRollingCells(new Set());
    setFeatureCells([...EMPTY_FEATURE_GRID]); setSelectedSymbol(null); setFeatureAttempt(0); setFeatureActive(false); setFeaturePurchased(false);
    setStoppedColumns(3); setBrakingColumn(-1); setLandingColumn(-1); setAnticipating(false);
  };

  const finishRoundPresentation = () => {
    setPhase("idle"); setFeatureActive(false); setFeaturePurchased(false); setFeatureCells([...EMPTY_FEATURE_GRID]); setSelectedSymbol(null);
    setFeatureAttempt(0); setRollingCells(new Set()); setFreshCells(new Set()); setStoppedColumns(3); setBrakingColumn(-1); setLandingColumn(-1); setAnticipating(false);
  ...
  const runSpin = useCallback(async (): Promise<boolean> => {
    ...
    if (!arcadeActions.placeBet(bet)) { busyRef.current = false; playGoldenTigerAudio({ type: "lose" }, soundEnabled); return false; }
    try {
      ...
      for (let column = 0; column < 3; column += 1) {
        if (column === 2 && (baseResult.winning.size > 0 || featureTriggered)) {
          setAnticipating(true); playGoldenTigerAudio({ type: "anticipation" }, soundEnabled); await wait(goldenTigerAnticipationMs(turbo));
        }
        setLandingColumn(column); setBrakingColumn(column); await wait(goldenTigerReelBrakeMs(column, turbo));
        setStoppedColumns(column + 1); setBrakingColumn(-1);
      ...
      setLandingColumn(-1); setBrakingColumn(-1); setAnticipating(false);

      if (featurePlan) {
        playGoldenTigerAudio({ type: "feature-open" }, soundEnabled);
        const payout = await animateFeature(featurePlan, false);
        await settle(payout, bet, bet, `3×3 · Fortune Feature ${SYMBOL_LABEL[featurePlan.selectedSymbol]} · ${featurePlan.respinsUsed} respin(s) · ${featurePlan.lines} linha(s)`, featurePlan.isFullGrid);
        return true;
      }

      setWinning(baseResult.winning); setPhase("reveal");
      if (baseResult.payout > bet) playGoldenTigerAudio({ type: "reveal", winMultiple: baseResult.payout / bet }, soundEnabled);
      await wait(turbo ? 55 : baseResult.winning.size > 0 ? 180 : 110);
      await settle(baseResult.payout, bet, bet, `3×3 · ${baseResult.lines} linha(s)${baseResult.isFullGrid ? ` · TELA CHEIA ×${GOLDEN_TIGER_FULL_GRID_MULTIPLIER}` : ""}`, baseResult.isFullGrid);
      return true;
    } finally { finishRoundPresentation(); }
  ...
  const buyBonus = useCallback(async () => {
    ...
    busyRef.current = true;
    setBonusOpen(false);
    if (!arcadeActions.debitCoins(bonusCost)) { busyRef.current = false; return; }
    try {
      resetRoundPresentation();
  ...
  }, [animateFeature, autoLeft, balance, bet, bonusCost, settle]);

  const startAuto = useCallback(async () => {
    if (busyRef.current || autoLeft > 0) return;
    autoStopRef.current = false; setAutoOpen(false);
    for (let left = autoRounds; left > 0; left -= 1) {
      if (autoStopRef.current) break;
      setAutoLeft(left); if (!(await runSpin())) break; await wait(turbo ? 90 : 250);
    }
    setAutoLeft(0);
  ...
  const status =
    ...
    phase === "win" ? `GANHO ${formatCoins(win)}` : "FORTUNE FEATURE PODE SURGIR A QUALQUER GIRO";

  return (
    <main className="gt-hw-page gt-premium-page">
      <section className="gt-hw-machine gt-premium-machine" data-phase={phase} data-feature-mode={featureActive ? "active" : "base"} data-feature-purchased={featurePurchased ? "true" : "false"} data-anticipating={anticipating ? "true" : "false"} aria-label="Golden Tiger">
        ...
        <header className="gt-hw-topbar">
          ...
          <div className="gt-hw-brand"><small>NEON FORTUNE ARCADE</small><h1>GOLDEN TIGER</h1><span>FORTUNE ROARS</span></div>
        ...
        <div className="gt-hw-grid gt-premium-grid" data-landing-column={landingColumn} data-reveal={phase === "reveal" ? "true" : "false"} aria-label="Grade de símbolos 3 por 3">
          {grid.map((symbol, index) => {
            ...
            return (
              <div className={cn("gt-hw-cell", winningCell && "is-winning", locked && "is-locked", freshCells.has(index) && "is-fresh", featureActive && featureSymbol === null && "is-feature-blank")} key={index}>
                {featureActive ? featureSymbol ? <GoldenTigerSymbol id={featureSymbol} isWinning={winningCell} /> : <span className="gt-hw-feature-blank" aria-hidden /> : <GoldenTigerSymbol id={symbol} isWinning={winningCell} />}
                {featureActive && selectedSymbol && rollingCells.has(index) && <FeatureCellMotion selectedSymbol={selectedSymbol} index={index} />}
                {locked && <span className="gt-premium-lock-ring" aria-hidden />}
              </div>
            );
          })}
          {phase === "base-spin" && [0, 1, 2].filter((column) => column >= stoppedColumns).map((column) => <ReelOverlay column={column} turbo={turbo} braking={brakingColumn === column} finalSymbols={reelFinalSymbols(grid, column)} key={column} />)}
        ...
        <div className="gt-hw-controls gt-premium-controls">
          ...
          <div className="gt-hw-secondary-controls gt-premium-secondary">
            ...
            <button type="button" className="gt-premium-bonus-button" onClick={() => setBonusOpen(true)} disabled={isBusy || autoLeft > 0 || balance < bonusCost}><Gift /><span>BÔNUS</span></button>
        ...
        {(phase === "bonus-intro" || phase === "feature-intro") && featureActive && (
          <div className="gt-premium-feature-title" aria-live="polite"><small>{featurePurchased ? "FEATURE COMPRADA" : "FEATURE ATIVADA"}</small><strong>FORTUNE FEATURE</strong><span>{selectedLabel} + WILD · SÍMBOLOS FICAM FIXOS</span></div>
        )}

        {phase === "win" && win > 0 && (winTier === "small" || winTier === "nice") && <div className={cn("gt-hw-win-ribbon", `is-${winTier}`)} aria-live="polite"><span>{winTier === "nice" ? "BOM GANHO" : "GANHO"}</span><AnimatedWinCounter value={win} duration={reducedMotion() ? 0 : 620} /></div>}
        {((phase === "win" && (winTier === "big" || winTier === "mega" || winTier === "super")) || phase === "full-grid") && win > 0 && <div className={cn("gt-hw-win-overlay", phase === "full-grid" && "is-full", (winTier === "mega" || winTier === "super") && "is-mega", winTier === "super" && "is-super")}><div className="gt-hw-win-rays" aria-hidden /><div className="gt-hw-win-crown" aria-hidden>✦</div><span>{phase === "full-grid" ? "TELA CHEIA" : winTier === "super" ? "SUPER MEGA GANHO" : winTier === "mega" ? "MEGA GANHO" : "GRANDE GANHO"}</span><AnimatedWinCounter value={win} duration={reducedMotion() ? 0 : 1500} />{phase === "full-grid" && <small>GANHOS × {FORTUNE_FEATURE_FULL_GRID_MULTIPLIER}</small>}</div>}

        {bonusOpen && (
          <div className="gt-hw-modal gt-premium-bonus-modal" role="dialog" aria-modal="true" aria-label="Comprar Fortune Feature">
            <div><span>FORTUNE FEATURE</span><h2>COMPRAR O BÔNUS?</h2><div className="gt-premium-bonus-medallion" aria-hidden>福</div><p>Entre diretamente na Fortune Feature. Um símbolo de fortuna é escolhido; ele e os WILDs ficam fixos enquanto novos respins continuarem chegando.</p><dl><div><dt>APOSTA BASE</dt><dd>{formatCoins(bet)}</dd></div><div><dt>CUSTO</dt><dd>{formatCoins(bonusCost)} · {BONUS_BUY_MULTIPLIER}×</dd></div><div><dt>TELA CHEIA</dt><dd>GANHOS ×{FORTUNE_FEATURE_FULL_GRID_MULTIPLIER}</dd></div></dl><footer><button type="button" onClick={() => setBonusOpen(false)}>CANCELAR</button><button type="button" className="is-buy" onClick={() => void buyBonus()} disabled={balance < bonusCost}>ATIVAR FEATURE</button></footer></div>
          </div>
        )}
