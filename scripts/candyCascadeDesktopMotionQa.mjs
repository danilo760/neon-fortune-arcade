...
async function waitFor(client, expression, label, timeout = 12000) {
  ...
  const debug = await evaluate(client, `(() => ({
    phase: document.querySelector('.ccp-machine')?.getAttribute('data-phase') ?? null,
...
function validate(audit, label) {
  ...
  const machine = audit.machine;
  for (const [name, state] of Object.entries(audit.sections ?? {})) {
    if (!state?.visible) errors.push(`${name} disappeared at ${label}`);
    const r = state?.rect;
    if (!r) continue;
    if (r.left < machine.left - 2 || r.right > machine.right + 2) errors.push(`${name} clipped horizontally at ${label}`);
    if (r.top < machine.top - 2 || r.bottom > machine.bottom + 2) errors.push(`${name} clipped vertically at ${label}`);
    if (name !== "mascot" && name !== "footer" && !state.centerHit) errors.push(`${name} center is obscured at ${label}`);
  }

  const allowVerticalRefillTravel = audit.phase === "refill";
  for (const [cellIndex, r] of (audit.cellRects ?? []).entries()) {
    if (!r || r.width < 20 || r.height < 20) {
      errors.push(`cell ${cellIndex + 1} collapsed at ${label}`);
      continue;
    }
    if (!audit.grid) continue;

    const horizontalEscape = r.left < audit.grid.left - 2 || r.right > audit.grid.right + 2;
    const verticalEscape = r.top < audit.grid.top - 2 || r.bottom > audit.grid.bottom + 2;
    if (horizontalEscape || (!allowVerticalRefillTravel && verticalEscape)) {
...
// Ordinary real-timing spin: keep screenshots because the phases are long enough
// and these images are useful for visual regression review.
{
...
// Deterministic 30-symbol cluster -> guaranteed Sugar Bomb. Forced phases are
// sampled without screenshots because bomb-birth/bomb-burst only last ~160 ms.
{
  ...
  try {
    ...
    for (const phase of ["cluster", "bomb-birth", "bomb-burst", "collapse", "refill"]) {
      await waitFor(
        scenario.client,
        `document.querySelector('.ccp-machine')?.getAttribute('data-phase') === ${JSON.stringify(phase)}`,
        `deterministic ${phase}`,
      );
      const audit = await evaluate(scenario.client, auditExpression);
      ...
      if (phase === "bomb-burst") {
        ...
        if (Number(audit.impactZ) >= Number(audit.sections.sugarHud?.zIndex ?? 0)) {
          errors.push(`impact layer z-index ${audit.impactZ} is not below persistent Sugar HUD`);
        }
      }
      if (errors.length) failed = true;
