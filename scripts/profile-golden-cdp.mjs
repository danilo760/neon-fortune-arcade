...
const APP = process.env.APP_URL ?? "http://127.0.0.1:4173/game/golden-tiger";
...
async function waitFor(cdp, expression, timeoutMs = 240000) { const start = Date.now(); while (Date.now() - start < timeoutMs) { if (await evaluate(cdp, expression)) return; await new Promise((resolve) => setTimeout(resolve, 120)); } throw new Error(`Timed out waiting for: ${expression}`); }
...
await cdp.send("Page.addScriptToEvaluateOnNewDocument", { source: `(() => { let a = 0x51f15e; Math.random = () => { a = (a + 0x6d2b79f5) >>> 0; let t = a; t = Math.imul(t ^ (t >>> 15), t | 1); t ^= t + Math.imul(t ^ (t >>> 7), t | 61); return ((t ^ (t >>> 14)) >>> 0) / 4294967296; }; })();` });
const loaded = cdp.once("Page.loadEventFired"); await cdp.send("Page.navigate", { url: APP }); await loaded;
await waitFor(cdp, `typeof window.__goldenRenderCount === 'number' && Boolean(document.querySelector('button[aria-label="Auto play"]'))`, 30000);
...
await evaluate(cdp, `window.__goldenRenderCount = 0; window.__goldenSymbolRenderCount = 0;`);
...
await evaluate(cdp, `(() => {
  ...
  const tick = (now) => {
    ...
    if (document.querySelector('button[aria-label="Parar auto play"]')) window.__perfData.activeSamples += 1;
...
await waitFor(cdp, `Boolean(document.querySelector('button[aria-label="Auto play"]')) && !document.querySelector('button[aria-label="Auto play"]')?.disabled`, 240000);
...
const result = await evaluate(cdp, `(() => {
  ...
  return { elapsedMs: elapsed, avgFps: elapsed > 0 ? data.frames / (elapsed / 1000) : 0, minFps: worst ? 1000 / worst : 0, p99Fps: p99 ? 1000 / p99 : 0, worstFrameMs: worst, p99FrameMs: p99, longTasks: data.longTasks.length, maxLongTaskMs: data.longTasks.length ? Math.max(...data.longTasks) : 0, renderCount: window.__goldenRenderCount ?? null, symbolRenderCount: window.__goldenSymbolRenderCount ?? null, activeSamples: data.activeSamples, maxDomNodes: data.maxNodes };
})()`);
...
if (!result.activeSamples || result.renderCount < 5) throw new Error(`Invalid Golden stress run: ${JSON.stringify(result)}`);
