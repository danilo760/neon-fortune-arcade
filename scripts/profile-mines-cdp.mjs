...
const APP = process.env.APP_URL ?? "http://127.0.0.1:4173/game/neon-mines";
...
await loaded;
await waitFor(cdp, `typeof window.__minesRenderCount === 'number' && Boolean(document.querySelector('button[aria-label^="Abrir cofre apostando"]'))`, 30000);
await waitFor(cdp, `!document.querySelector('button[aria-label^="Abrir cofre apostando"]')?.disabled`, 30000);
await evaluate(cdp, `window.__minesRenderCount = 0;`);
const before = await collect(cdp);
await evaluate(cdp, `(() => {
  window.__perfStop = false;
  window.__perfData = { start: performance.now(), frames: 0, deltas: [], longTasks: [], maxNodes: 0, activeSamples: 0 };
  ...
  const tick = (now) => {
    ...
    if (document.querySelector('button[aria-label^="Cash out por"]')) window.__perfData.activeSamples += 1;
...
for (let round = 0; round < 5; round += 1) {
  await evaluate(cdp, `document.querySelector('button[aria-label^="Abrir cofre apostando"]')?.click()`);
  await waitFor(cdp, `Boolean(document.querySelector('button[aria-label="Revelar casa 25"]'))`, 5000);
  for (const cell of [25, 24, 23]) {
    await waitFor(cdp, `!document.querySelector('button[aria-label="Revelar casa ${cell}"]')?.disabled`, 10000);
    await evaluate(cdp, `document.querySelector('button[aria-label="Revelar casa ${cell}"]')?.click()`);
    await waitFor(cdp, `Boolean(document.querySelector('button[aria-label="Casa ${cell}, segura"]')) && !document.querySelector('button[aria-label^="Cash out por"]')?.disabled`, 10000);
  }
  await evaluate(cdp, `document.querySelector('button[aria-label^="Cash out por"]')?.click()`);
  await waitFor(cdp, `Boolean(document.querySelector('button[aria-label^="Abrir cofre apostando"]')) && !document.querySelector('button[aria-label^="Abrir cofre apostando"]')?.disabled`, 10000);
}
...
const result = await evaluate(cdp, `(() => {
  ...
  const sorted = data.deltas.filter((value) => value > 0).sort((a,b) => a-b);
  const worst = sorted.at(-1) ?? 0;
  const p99 = sorted.length ? sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * .99))] : 0;
  return {
    elapsedMs: elapsed,
    avgFps: elapsed > 0 ? data.frames / (elapsed / 1000) : 0,
    minFps: worst ? 1000 / worst : 0,
    p99Fps: p99 ? 1000 / p99 : 0,
    worstFrameMs: worst,
    p99FrameMs: p99,
    longTasks: data.longTasks.length,
    maxLongTaskMs: data.longTasks.length ? Math.max(...data.longTasks) : 0,
    renderCount: window.__minesRenderCount ?? null,
    estimatedTileRenderOps: (window.__minesRenderCount ?? 0) * 25,
    activeSamples: data.activeSamples,
    maxDomNodes: data.maxNodes,
  };
})()`);
const after = await collect(cdp);
result.jsHeapBefore = metric(before, "JSHeapUsedSize");
result.jsHeapAfter = metric(after, "JSHeapUsedSize");
result.jsHeapDelta = result.jsHeapAfter - result.jsHeapBefore;
result.taskDuration = metric(after, "TaskDuration") - metric(before, "TaskDuration");
result.scriptDuration = metric(after, "ScriptDuration") - metric(before, "ScriptDuration");
result.layoutDuration = metric(after, "LayoutDuration") - metric(before, "LayoutDuration");
result.recalcStyleDuration = metric(after, "RecalcStyleDuration") - metric(before, "RecalcStyleDuration");
